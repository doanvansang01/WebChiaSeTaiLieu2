				...
				<form class="user" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
					<div class="auth-form__form-input">
						...
						<div class="auth-form__group">
							<input type="text" class="auth-form__input" placeholder="Nhập họ tên" name="name" value="<?php if (isset($_POST['btnSignin'])) echo $_POST["name"]; ?>">
						</div>
						<span class="error"><?php if (isset($error['name'])) echo $error['name']; ?></span>
						<div class="auth-form__group">
							<input type="date" class="auth-form__input" placeholder="Chọn ngày sinh" name="date" value="<?php if (isset($_POST['btnSignin'])) echo $_POST["date"]; ?>">
						</div>
						<span class="error"><?php if (isset($error['date'])) echo $error['date']; ?></span>
						<div class="auth-form__group">
							<b>CHỌN GIỚI TÍNH:</b>
							<input class="rdb" type="radio" name="gender" value="1" checked="checked" /><b>Nam</b>
							<input class="rdb" type="radio" name="gender" value="0" /><b>Nữ<b>
						</div>
						<?php
						if (isset($_POST['btnSignin'])) {
						...
						?>
						<div class="auth-form__group">
							<input type="text" class="auth-form__input" placeholder="Nhập email" name="email" value="<?php if (isset($_POST['btnSignin'])) echo $_POST["email"]; ?>">
						</div>
						<span class="error"><?php if (isset($error['email'])) echo $error['email']; ?></span>
						<div class="auth-form__group">
							<input type="password" class="auth-form__input" placeholder="Nhập mật khẩu" name="pass" value="<?php if (isset($_POST['btnSignin'])) echo $_POST["pass"]; ?>">
						</div>
						<span class="error"><?php if (isset($error['pass'])) echo $error['pass']; ?></span>
						<div class="auth-form__group">
							<input type="password" class="auth-form__input" placeholder="Nhập lại mật khẩu" name="repass" value="<?php if (isset($_POST['btnSignin'])) echo $_POST["repass"]; ?>">
						</div>
